...
<body>
	...
	<!-- 메인 화면 -->
	<div class="hoc clear wrapper row3" style="margin-left:15px; margin-top:15px;">
		<!-- main body -->
		...
		<div class="one_half first" style="width: 55%;padding-right: 10px;">
			...
			<div class="bodyArea">
				...
				<div class="" style="margin-top:15px; height: 60px">
                      ...
                      ${tagAttribute}="${propertyType.getTagAttributeMapValue(tagAttribute)}"
														...
														onchange="modSubPropertyTypes(this)"> <label
														class="form-check-label">${propertyType.name} 전체</label>
												</div>

												<button class="btn btn-primary dropdown-toggle"
													type="button" data-bs-toggle="collapse"
													data-bs-target="#collapse-${propertyType.id}"
													aria-expanded="false"
													aria-controls="collapse-${propertyType.id}">
													${propertyType.name} 세부유형</button> <c:set var="isChecked"
													value="${false}" />
												<div class="collapse" id="collapse-${propertyType.id}">
													<c:forEach var="subPropertyType"
														items="${propertyType.subPropertyTypes}">
														<div class="form-check form-check-inline">
															<input type="checkbox"
																id="subPropertyType-${propertyType.id}-${subPropertyType.id}"
                          ...
                          <c:if test='${tagAttribute == "checked"}'>
                            <c:set var='isChecked' value='${true}'/>
																...
																class="form-check-input" name="subPropertyTypeId"
																value="${subPropertyType.id}"> <label
																class="form-check-label">${subPropertyType.name}</label>
														</div>
													</c:forEach>
												</div> <c:if test="${isChecked}">
													<script type="text/javascript">
														document
																.querySelector(
																		"div#collapse-${propertyType.id}")
																.setAttribute(
																		"class",
																		"collapse show");
													</script>
										...
										<li><input type="button" class="btn btn-secondary"
											value="초기화" onclick="resetTags('propertyTypes')"></li>
									</ul>
								</div>
							...
							<div id="btn_group" class="btn-group">
								...
								<ul class="dropdown-menu list-group"
									aria-labelledby="dropdownMenuClickableInside-2">
									<c:forEach var="roomType" items="${roomTypes}">
										<li class="list-group-item" style="font-size: 20px">
											<div class="form-check form-check-inline">
												<input type="checkbox" class="form-check-input"
													name="roomTypeId" value="${roomType.id}"
                      ...
                      ${tagAttribute}="${roomType.getTagAttributeMapValue(tagAttribute)}"
									...
									<li><input type="button" class="btn btn-secondary"
										value="초기화" onclick="resetTags('roomTypes')"></li>
								</ul>
							</div>
							...
							<div id="btn_group" class="btn-group">
								...
								<button class="btn btn-secondary btn-lg dropdown-toggle"
									type="button" id="dropdownMenuClickableInside-3"
									data-bs-toggle="dropdown" data-bs-auto-close="outside"
									aria-expanded="false" style="font-size:20px; padding-left:15px; padding-right:15px;">편의시설</button>

								<div class="dropdown-menu"
									aria-labelledby="dropdownMenuClickableInside-3"
									style="font-size: 15px">
									<c:forEach var="amenityType" items="${amenityTypes}">
										<div class="form-check form-check-inline">
											<input type="checkbox" class="form-check-input"
												name="amenityTypeId" value="${amenityType.id}"
                    ...
                    ${tagAttribute}="${amenityType.getTagAttributeMapValue(tagAttribute)}"
									...
									<input type="button" class="btn btn-secondary" value="초기화"
										onclick="resetTags('amenityTypes')">
								</div>
							...
							<div id="btn_group" class="btn-group">
								...
								<div class="dropdown-menu list-group"
									aria-labelledby="dropdownMenuClickableInside-4"
									style="width: 500px;">
									<div class="btn-group list-group-item"
										style="padding-bottom: 50px; padding-left: 20px">
										<h3>인원</h3>
										<input id="decrease-guest" type="button" class="btn" value="-"
											onclick="changeCount(this)" style="font-size: 20px;">
									...
									<div class="list-group-item form-check form-check-inline"
										...
										~&nbsp; ₩<input id="max-price"
											class="form-control form-check-label" type="number"
									...
									<div class="list-group-item">
										<input type="button" class="btn btn-secondary" value="초기화"
											onclick="resetTags('etc')">
									</div>
								</div>
							</div>
						</div>
						<input type="button" class="btn btn-secondary" value="전체 초기화"
							onclick="resetTags('all')">
			</div>
			</form>
			<!-- 숙소리스트 영역 -->
			<hr>
				...
				<c:forEach var="property" items="${properties}">
					...
					<li style="height: 150px;">
						...
						<div class="one_third first">
							...
							<div id="carouselControls-${property.id}" class="carousel slide">
								...
								<c:if
									test="${not empty property.images && property.images.size() > 1}">
									<button class="carousel-control-prev" type="button"
										data-bs-target="#carouselControls-${property.id}"
										data-bs-slide="prev">
						...
						<div class="two_third position-relative">
							...
							<h2>
								<a
									href="<c:url value='/property/detail?propertyId=${property.id}'/>">${property.name}</a>
							</h2>
					...
					<div class="position-relative">
						<div class="position-absolute end-0 bottom-50">
							...
							<a href="" class="trigger-btn wish-button"
								id="wishProperty-${property.id}" data-toggle="modal"> <!-- 빈 하트 -->
								<img class="heart" src="" style="width: 3rem; height: 3rem">
							</a>
							<script type="text/javascript">
								// 화면 로드 시 초기화하는 과정
								initWish("${property.id}",
										"${property.wishListId}",
										"${property.wished}");
							</script>
						</div>
	...
	<div class="position-relative" style="margin-left:170px">
		...
		<div class="one_half" style="margin-left:0px; width: 40%;">
			...
			<script type="text/javascript"
				src="//dapi.kakao.com/v2/maps/sdk.js?appkey=62b11c585fb341eec39dbc28ac9bad71"></script>
						...
						.ready(
								function() {
									...
									function displayMarker(data) {
										// 마커 이미지 크기
										var imageSize = new kakao.maps.Size(38,
												40);

										// 마커 이미지 생성    
										var markerImage = new kakao.maps.MarkerImage(
												"/resources/property_img/marker.png",
												imageSize);
										var overMarkerImage = new kakao.maps.MarkerImage(
												"/resources/property_img/marker2.png",
												imageSize);
										var selectedMarkerImage = new kakao.maps.MarkerImage(
												"/resources/property_img/marker1.png",
												imageSize);
										...
										//오버레이 안의 이미지 구현
										var imageArea = document
												.createElement('div');
										imageArea.id = 'carouselControls-img'
												+ data.id;
										...
										desc.innerHTML = '<img src="/resources/property_img/starIcon.PNG" style="width:18px; height:18px; margin-bottom:5px;">4.84<br>'
												+ '<a href="/property/detail?propertyId='
												+ data.id
												+ '" target="_blank">'
												+ data.propertyType
												+ '<br>'
...
<div class="wrapper row5" style="margin-top:;">
