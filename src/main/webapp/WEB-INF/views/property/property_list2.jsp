...
<style>
    ...
    .info .desc {position: relative;height: 90px; width:300px; padding:10px; border: 1px solid #ccc;font-size:15px;}
    .info .img {position: relative;width: 295px;height: 220px;overflow: hidden;}
	      	...
	      	$(document).ready(function(){
		     	...
		     	for (var i=0; i<positions.length; i++) {
		      	    ...
		      	    var marker = new kakao.maps.Marker({
		      	        ...
		      	        id : id, //숙소id
		      	        title : positions[i].name, // 마커의 타이틀(숙소명)
		      	    ...
		      	    const content = makeContent(positions[i].id, positions[i].propertyType, positions[i].title);
				    ...
				    kakao.maps.event.addListener(marker, 'click', function() {
				    	// 커스텀 오버레이 생성
				    	 const customOverlay = new kakao.maps.CustomOverlay({
					         position : position, //마커의 포지션에 생성
					         content : content,
					         //clickable: true, //지도이벤트 막아줌
					         map : map 
					     });
		     	...
		     	// 오버레이에 출력될 숙소 정보 만드는 함수
				function makeContent(id, propertyType, title){
					if(title.length>20){
		            ...
		            '            			</div>' +  
		            '            			<div class="desc">' + 
		            '               			<img src="/resources/property_img/starIcon.PNG" style="width:18px; height:18px; margin-bottom:5px;">4.84<br>' +
		            '                			<a href="/property/detail?propertyId='+ id + '" target="_blank">' +
		            '				 			' + propertyType + '<br>'+ title + 
		            '				 			</a>' + 
		            '            			</div>' + 
		            '        			</div>' + 
		            '    			</div>' +    
